 ...
 * @author jsongo
 * @date 2019/8/1 23:11
 * @desc copy from RxTool -> ActivityScanerCode
 */
public class ScanCodeActivity extends AppCompatActivity {
     ...
     * 扫描处理
     */
    private ScanCodeActivity.CaptureActivityHandler handler;
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        ...
        if (handler == null) {
            handler = new ScanCodeActivity.CaptureActivityHandler();
        }
    }
    ...
    final class CaptureActivityHandler extends Handler {
        ...
        ScanCodeActivity.DecodeThread decodeThread = null;
        private ScanCodeActivity.State state;

        public CaptureActivityHandler() {
            decodeThread = new ScanCodeActivity.DecodeThread();
            decodeThread.start();
            state = ScanCodeActivity.State.SUCCESS;
        ...
        public void handleMessage(Message message) {
            if (message.what == R.id.auto_focus) {
                if (state == ScanCodeActivity.State.PREVIEW) {
                ...
                state = ScanCodeActivity.State.PREVIEW;
                CameraManager.get().requestPreviewFrame(decodeThread.getHandler(), R.id.decode);
            }
        }

        public void quitSynchronously() {
            state = ScanCodeActivity.State.DONE;
        ...
        private void restartPreviewAndDecode() {
            if (state == ScanCodeActivity.State.SUCCESS) {
                state = ScanCodeActivity.State.PREVIEW;
                CameraManager.get().requestPreviewFrame(decodeThread.getHandler(), R.id.decode);
                CameraManager.get().requestAutoFocus(this, R.id.auto_focus);
            }
    ...
    final class DecodeThread extends Thread {
        ...
        public void run() {
            Looper.prepare();
            handler = new ScanCodeActivity.DecodeHandler();
